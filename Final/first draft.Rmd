...
fake_ideology_matrix_2[3,2] <- fake_ideology_matrix_2[3,2] + 6
fake_ideology_matrix_2[1,2] <- fake_ideology_matrix_2[1,2] + 3
fake_ideology_matrix_2[4,2] <- fake_ideology_matrix_2[4,2] + 2
#                  voted2_refused_count
# Conservative 527                   38
# Don't Know    68                    6
# Liberal      424                   27
...
In fact, it would only be necessary for 6 of the 14 "Refused" voters to be Liberal to push the p-values for our analysis above 0.05, even if the remaining 5 were distributed in approximately equal proportions among the remaining choices (3 to Conservative and 2 to Moderate)[^data_note_1]. So even a slight Liberal tilt in these voters would be enough to prevent us from supporting our initial hypothesis, and the answers we saw to the questions above provide fairly strong support for the existence of this bias.

A final note is that it is possible that the now-hypothesized liberal bias among the respondents to this survey who did not disclose their ideology fits within the overall trend, which we saw at the beginning, of stronger-than-national-average support for Joe Biden among the respondents in general. If Joe Biden supporters tend liberal, and if more Biden supporters than Trump supporters participated in this survey (as suggested by the data), then it would be a reasonable supposition that all subsets of the data would likely trend in the same direction. Without perfect knowledge of all participants' ideologies, we can only guess, but this is not part of our hypothesis.
...
It also does not *disprove* this possibility, of course. But these results could be surprising to those (including this author) who accepted the general notion of some degree of Conservative bias in undisclosed ideological preferences among voters. It is still possible that this bias exists, but this examination of this dataset does not support it.
...
Some of the time-sinks would be expected in any circumstance of bootstrapping onself into a new area of knowledge; I have a long background in computer programming in various languages dating back to the early 1980s, but they are/were mostly low-level languages initially (C and assembly), and in more recent years, Perl, Objective-C, PHP and Swift, so I have become increasingly used to strongly-typed languages (or the ability to activate strong typing in more permissive languages), and R is pretty different in some important ways. Although the basics of the language are straightforward, some of the nuances about underlying data structures are quite complex (like the difference between vectors, data frames, tables and matrices, and their modes of access), and without strict typing, it's possible to make errors that are difficult to detect.

In particular, I got hung up pretty hard on the proper way to run `chisq.test` on frequency tables from the data, and initially was getting an erroneous result due to bad inputs rather than bad data. My case was specific enough that doing productive online searches was grueling and probably more confusing than helpful; I ended up rolling back to a completely manual data set and doing a lot of experimenting to first get the correct results, and then a lot of work to reverse-engineer the manual data back to the proper references for the actual data set. Someone more experienced than me in R could probably have identified my issue immediately, but it took a long time to find on my own.
...
I can see at this point how powerful deep fluency with data query languages like R, and with statistical principles, will be in future research. I have a lot to learn about R still, and about statistical methods, but I need no convincing on their utility. I look forward to continuing to develop both in future classes as I move through the program.
